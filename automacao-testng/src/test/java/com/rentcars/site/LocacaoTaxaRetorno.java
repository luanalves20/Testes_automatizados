...
public class LocacaoTaxaRetorno {
	...
	@Test
	public void locacaoTaxaRetorno() throws InterruptedException {
		...
		Thread.sleep(500);
		element = driver.findElement(By.id("dropLocation"));
		element.click();

		element = driver.findElement(By.id("LocalidadeDevolucao"));
		element.sendKeys("Rio de Janeiro");
		...
		if (statusPedido.equals("Aguardando Aprovação")) {
			...
			if (validaTipoPagamento == true) {
				try {
					...
					System.out.println("Elemento pagamento cartão não encontrado");
				}
			} else {
				wait.until(ExpectedConditions
						.visibilityOfElementLocated(By.xpath("//div[@class='box_s_reg codigoPedido']/h2/b")));
		...
		try {
			gravaArquivosLog.GravaLocacaoTaxaRetorno(elementos.getNumeroReserva());
		} catch (IOException e) {
			e.printStackTrace();
		}
